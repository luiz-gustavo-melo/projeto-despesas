...
BEGIN TRY
	...
	-- usuário existente;
	ELSE IF (EXISTS (SELECT 1 FROM [dbo].[Usuarios] WHERE (NOME_USUARIO = @NOME_USUARIO) OR (EMAIL = @EMAIL)))
		BEGIN 
	...
	ELSE
		BEGIN
			INSERT [dbo].[Usuarios] (NOME_USUARIO, EMAIL, SENHA)
...
SELECT 1 FROM Usuarios WHERE (NOME_USUARIO = 'LuizMelo') OR (EMAIL = 'luiz@email.com')
*/